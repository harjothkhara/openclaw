...
it("continues with unregistered staging when Windows identity admission is unknown", async () => {
  ...
  expect(plan.warnings).toEqual([expect.stringContaining("automatic recovery is disabled")]);
  expect(log).toHaveBeenCalledWith(expect.stringContaining("automatic recovery is disabled"));
  await fs.writeFile(plan.tempArchivePath, "unproven payload");
  await cleanupBackupArchivePublication(plan, log);
  const next = await create(root, "next.tar.gz");
  await expect(fs.readFile(plan.tempArchivePath, "utf8")).resolves.toBe("unproven payload");
  await cleanupBackupArchivePublication(next);
});
